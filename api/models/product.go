...
type ProductInfo struct {
	...
	TwoCategoryID   int16            `form:"two_category_id" binding:"required" gorm:"column:two_category_id;type:smallint(5) unsigned;not null"`     // 二级分类ID
	ThreeCategoryID int16            `form:"three_category_id" binding:"required" gorm:"column:three_category_id;type:smallint(5) unsigned;not null"` // 三级分类ID
	Price           float64          `form:"price" binding:"required" gorm:"column:price;type:decimal(8,2);not null"`                                 // 商品销售价格
	AverageCost     float64          `form:"average_cost" binding:"required" gorm:"column:average_cost;type:decimal(18,2);not null"`                  // 商品加权平均成本
	...
	ShelfLife       int              `form:"shelf_life" gorm:"column:shelf_life;type:int(11);not null"`            // 商品有效期
	Descript        string           `form:"descript" gorm:"column:descript;type:text;not null"`                   // 商品描述
	Indate          string           `form:"indate" gorm:"column:indate;type:timestamp;not null"`                  // 商品录入时间
